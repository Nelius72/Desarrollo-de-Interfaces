    ...
    public partial class Form1 : Form
    {
        ...
        private void textBox1_TextChanged(object sender, EventArgs e)
        {
            if (textBox1.Text.Length != 0)
            {
                ...
                { //Recorremos las filas del DGV
                    string nombre = dataGridView1.Rows[i].Cells[0].Value + "";
                    if (nombre.Contains(textBox1.Text))
                    {
                        dataGridView1.Rows[i].Visible = true;
                    }
                    else if (!dataGridView1.Rows[i].IsNewRow)
                    {
            ...
            {
                for (int n = 0; n < dataGridView1.Rows.Count - 1; n++)//Recorremos el dgv cuando en el filtro
                    //no haya nada y nos muestra de nuevo el contenido.
                {
                    dataGridView1.Rows[n].Visible = true;
                }
